...
type IShow interface {
	...
	GetWarnInfo() ([]model.HistoryResponse, error)
	// GetWarnInfoWithId 根据id返回告警信息
	GetWarnInfoWithId(id int32) ([]model.HistoryResponse, error)
	// GetWarnInfoWithParams 带参数查询告警信息, 参数为Ip和Local、Level、start、end
	GetWarnInfoWithParams(hinfo model.HistoryInfo) ([]model.HistoryResponse, error)
