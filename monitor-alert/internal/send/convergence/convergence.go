...
func (c *Convergence) AlertConvergence(target string, alert *model2.AlertInfo) {
...
func (c *Convergence) push(data Node) {
	c.stack.EnQueue(data)
}

func (c *Convergence) pop() Node {
...
// 获取所有的告警信息
func (c *Convergence) allData() []Node {
	list := make([]Node, 0)
	c.lock.Lock()
	for {
		data := c.pop()
